...
class RestaurantDefaultImageController extends Controller
{
    ...
    /**
     * Store the menu item placeholder image
     */
    public function storeMenuPlaceholder(Request $request, $slug)
    {
        $restaurant = Restaurant::where('slug', $slug)->firstOrFail();
        
        // Check authorization
        if (!\App\Models\Manager::canAccessRestaurant(Auth::id(), $restaurant->id, 'manager') && !Auth::user()->isAdmin()) {
            abort(403, 'Unauthorized access to restaurant menu placeholder image.');
        }
        ...
        if (!$restaurant->canSetCustomDefaultImage()) {
            return response()->json([
                'success' => false,
                'message' => 'Custom menu placeholder images are a Premium feature. Please upgrade your subscription.'
            ], 403);
        }
        
        $request->validate([
            'menu_placeholder_image' => 'required|image|mimes:jpeg,png,jpg,gif,webp|max:5120', // 5MB max
        ]);
        
        try {
            // Delete old menu placeholder image if exists
            if ($restaurant->default_menu_image && Storage::disk('public')->exists($restaurant->default_menu_image)) {
                Storage::disk('public')->delete($restaurant->default_menu_image);
            }
            
            // Store the new menu placeholder image
            $image = $request->file('menu_placeholder_image');
            $originalName = $image->getClientOriginalName();
            $fileName = 'menu_placeholder_' . time() . '_' . uniqid() . '.' . $image->getClientOriginalExtension();
            ...
            // Update restaurant record
            $restaurant->update([
                'default_menu_image' => $storedPath
            ]);
            
            Log::info('Restaurant menu placeholder image uploaded successfully', [
                'restaurant_id' => $restaurant->id,
                'file_name' => $fileName,
                'file_size' => $image->getSize()
            ]);
            ...
            return response()->json([
                'success' => true,
                'message' => 'Menu placeholder image uploaded successfully!',
                'image_url' => $restaurant->fresh()->default_image_url
            ]);
            
        } catch (\Exception $e) {
            Log::error('Failed to upload restaurant menu placeholder image', [
                'restaurant_id' => $restaurant->id,
                'error' => $e->getMessage()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to upload menu placeholder image: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    /**
     * Remove the menu placeholder image
     */
    public function destroyMenuPlaceholder($slug)
    {
        $restaurant = Restaurant::where('slug', $slug)->firstOrFail();
        ...
        try {
            // Delete the menu placeholder image file
            if ($restaurant->default_menu_image && Storage::disk('public')->exists($restaurant->default_menu_image)) {
                Storage::disk('public')->delete($restaurant->default_menu_image);
            }
            ...
            // Update restaurant record
            $restaurant->update([
                'default_menu_image' => null
            ]);
            
            Log::info('Restaurant menu placeholder image removed', [
                'restaurant_id' => $restaurant->id
            ]);
            
            return response()->json([
                'success' => true,
                'message' => 'Menu placeholder image removed successfully!'
            ]);
            
        } catch (\Exception $e) {
            Log::error('Failed to remove restaurant menu placeholder image', [
                'restaurant_id' => $restaurant->id,
                'error' => $e->getMessage()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to remove menu placeholder image: ' . $e->getMessage()
            ], 500);
        }
    }
